...
APP_BUNDLE="${APP_EXECUTABLE%/Contents/MacOS/MacLoupeExample}"
LOUPE_MACOS_INJECTOR_PATH="$INJECTOR_PATH" .build/debug/loupe app launch \
  --bundle-id dev.loupe.macos-example \
  --macos-app "$APP_BUNDLE" \
  --port "$PORT" \
  --timeout 30
APP_PID="$(pgrep -f "$APP_EXECUTABLE" | tail -1 || true)"
if [[ -z "$APP_PID" ]]; then
  echo "error: CLI launched MacLoupeExample but its process could not be found" >&2
  exit 1
fi
cleanup() {
